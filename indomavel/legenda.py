...
def trechos_de_frases(frases, inicio, fim, max_palavras=MAX_PALAVRAS_TRECHO):
    """Gera trechos palavra por palavra proporcionais a partir das frases do Chub."""
    saida = []
    for frase in frases or []:
        f_inicio = float(frase.get("inicio", 0.0))
        f_fim = float(frase.get("fim", 0.0))
        if f_fim <= inicio or f_inicio >= fim:
            continue
        texto = frase.get("texto") or ""
        palavras = texto.split()
        if not palavras:
            continue
        por_vez = max(1, int(max_palavras))
        grupos = [palavras[i:i + por_vez] for i in range(0, len(palavras), por_vez)]
        duracao = max(0.1, f_fim - f_inicio)
        passo = duracao / max(1, len(grupos))
        for idx, grupo in enumerate(grupos):
            a = max(f_inicio + idx * passo, inicio)
            b = min(f_inicio + (idx + 1) * passo, fim)
            if b - a >= 0.05:
                saida.append({
                    "inicio": round(a, 3),
                    "fim": round(b, 3),
                    "palavras": grupo,
                    "destaque": _destaque(grupo),
                })
    return saida
