...
def encoder_video():
    """Detecta se h264_nvenc está operacional; senão, usa libx264."""
    global _ENCODER_H264
    if _ENCODER_H264 is not None:
        return list(_ENCODER_H264)
    ffmpeg_bin = config.FFMPEG or "ffmpeg"
    try:
        resultado = subprocess.run(
            [ffmpeg_bin, "-f", "lavfi", "-i", "color=c=black:s=256x256:d=0.1", "-c:v", "h264_nvenc", "-f", "null", "-"],
            capture_output=True, text=True, timeout=5,
        )
        if resultado.returncode == 0:
            _ENCODER_H264 = ["-c:v", "h264_nvenc", "-preset", "p4", "-cq", "20"]
            return list(_ENCODER_H264)
    except Exception:
        pass
    _ENCODER_H264 = ["-c:v", "libx264", "-preset", "fast", "-crf", "20"]
    return list(_ENCODER_H264)
...
def layout(formato, estilo):
    ...
    # Formato 16:9 é master original limpo, nunca sobrepõe card no topo
    if formato != "16:9" and estilo["card"] and (tag or headline):
        y = margem_topo
        if tag:
...
def _notificar(avisar, mensagem, progresso=None, etapa=None, etapa_nome=None, eta_s=None):
    if not avisar:
        return
    try:
        avisar(mensagem, progresso=progresso, etapa=etapa, etapa_nome=etapa_nome, eta_s=eta_s)
    except TypeError:
        try:
            avisar(mensagem)
        except Exception:
...
def filtro_ffmpeg(dados_layout, estilo, largura_fonte, altura_fonte, com_legenda):
    ...
    cortar_topo_px = round(estilo.get("cortar_topo", 0) * dados_layout["escala"])
    altura_alvo = area["h"] + cortar_topo_px
    escala = max(area["w"] / largura_fonte, altura_alvo / altura_fonte) * estilo["zoom"]
    largura_escalada = max(area["w"], math.ceil(largura_fonte * escala / 2) * 2)
    altura_escalada = max(altura_alvo, math.ceil(altura_fonte * escala / 2) * 2)
    folga_x = largura_escalada - area["w"]
    folga_y = max(0, altura_escalada - area["h"] - cortar_topo_px)
    corte_x = round(folga_x / 2 * (1 + estilo["enquadramento_x"]))
    corte_y = cortar_topo_px + round(folga_y / 2 * (1 + estilo["enquadramento_y"]))
    corte_x = max(0, min(folga_x, corte_x))
    corte_y = max(cortar_topo_px, min(altura_escalada - area["h"], corte_y))
    partes = [
...
def exportar(youtube_id, inicio, fim, formato, estilo, trechos, pasta_saida, titulo, ao_progredir=None, nome_arquivo=None, caminho_fonte=None):
    ...
    with tempfile.TemporaryDirectory(prefix="indomavel_") as temporaria:
        _notificar(ao_progredir, "Baixando o trecho do YouTube…", progresso=5.0, etapa="baixando", etapa_nome="Baixando trecho do YouTube (1080p)")
        ...
        def ao_baixar(dados):
            if dados.get("status") == "downloading":
                ...
                pct = (baixado / total * 30.0) if total > 0 else 15.0
                eta = dados.get("eta")
                _notificar(
                    ao_progredir,
                    f"Baixando trecho do YouTube… {min(35, int(5 + pct))}%",
                    progresso=round(min(35.0, 5.0 + pct), 1),
                    etapa="baixando",
                    etapa_nome="Baixando trecho do YouTube",
                    eta_s=eta,
                )
            elif dados.get("status") == "finished":
                _notificar(
                    ao_progredir,
                    "Trecho baixado, gerando moldura e legendas…",
                    progresso=35.0,
                    etapa="processando",
        ...
        _notificar(ao_progredir, "Iniciando renderização 1080p…", progresso=40.0, etapa="renderizando", etapa_nome="Iniciando renderização 1080p")
        comando = [
        ...
        t_inicio_ffmpeg = time.time()
        for linha in processo.stdout:
            if linha.startswith("out_time_us=") and linha.strip().split("=")[1].isdigit():
                feito = int(linha.strip().split("=")[1]) / 1_000_000
                frac = min(1.0, max(0.0, feito / duracao)) if duracao > 0 else 0.5
                pct = 40.0 + frac * 58.0
                t_passado = time.time() - t_inicio_ffmpeg
                eta = round((t_passado / frac) * (1.0 - frac)) if frac > 0.05 else None
                _notificar(
                    ao_progredir,
                    f"Renderizando 1080p… {min(99, int(pct))}%",
                    progresso=round(min(99.0, pct), 1),
                    etapa="renderizando",
        ...
        _notificar(ao_progredir, "Finalizando arquivo…", progresso=99.0, etapa="processando", etapa_nome="Salvando arquivo final")
        if nome_arquivo:
            nome = nome_arquivo if nome_arquivo.endswith(".mp4") else f"{nome_arquivo}.mp4"
        else:
        ...
        _notificar(ao_progredir, "Pronto para publicar!", progresso=100.0, etapa="concluido", etapa_nome="Pronto para publicar", eta_s=0)
    return destino
