...
class TestSrtSeparado(unittest.TestCase):
    ...
    def test_trechos_de_frases_gera_trechos_curtos(self):
        frases = [
            {"inicio": 10.0, "fim": 15.0, "texto": "Renan Santos fala com clareza sobre o futuro da missão"},
        ]
        trechos = legenda.trechos_de_frases(frases, 10.0, 15.0, max_palavras=3)
        self.assertTrue(len(trechos) >= 2)
        for t in trechos:
            self.assertTrue(10.0 <= t["inicio"] < t["fim"] <= 15.0)
            self.assertTrue(len(t["palavras"]) <= 3)
            self.assertIsNotNone(t["destaque"])
